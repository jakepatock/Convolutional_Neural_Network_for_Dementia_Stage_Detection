...
Training incorporated an early stopper (with a pacients of 50 epochs) based on maximizing the F1 score on the validation set. The final F1 score on the validation set was 0.9929654070946043, with an accuracy of 0.99296875 and loss of 0.0329371975162843. The model achieved an accuracy and loss of 1.0 and 0.0004381921370087527 on the training data. The model reach its max F1 score at epcoh 65 and this is the model state saved in the model.pth file. The figure "CNN_Eval_Plot_norm_batch16.png" in the data directory showcases a comparison plot between the model's training and validation accuracy and loss, along with a plot of the F1 scores during training. There was a difference in loss in the training vs the validation data giving evidence to the model overfitting on the training data. The extend of this overfitting is unknown due to my access to test data is currently limited. If this model would be deployed in any relevant usecase the issue of overfitting would need to be addressed. The red dot on the plot shows the saved models final statistics. The plot is shown below. The two other figures in the plots directory show the results of training the model with batch sizes of 32 and 64 images istead of 16. The microbatching and batch normalization stabalized the model's learning and was ultimately used to created the saved model. ![Alt text](https://github.com/jakepatock/Convolutional_Neural_Network_for_Dementia_Stage_Detection/blob/master/model_results/plots/CNN_Eval_Plot_norm_batch16.png)
    <br />
